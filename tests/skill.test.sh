...
for s in 'hub.sh plan' 'hub.sh run-start' 'hub.sh create-job' 'hub.sh article' 'hub.sh schedule' 'hub.sh run-finish' 'state.json' '--resume' 'at most 4' 'Agent failure policy' '"failed"' 'missingLanguages' 'topicId'; do
