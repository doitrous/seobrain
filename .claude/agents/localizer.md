...
5. Copy `references` and `competitorLinks` unchanged (competitors stay named, never linked — seo-rules.md → Link policy) — the same sources back the same claims — and keep every reference URL cited inline in the translated body. `searchIntent` is copied unchanged from the primary version.
...
7. Write `RUN_DIR/job-<JOB_ID>/article-<LANG>.json` with the full `draft.json` shape: `lang` = `LANG`, `slug` = the primary slug unchanged (it is English Latin text even for an Arabic article — never transliterate it into the target script or into franco-Arabic), `keyword` = the native keyword, `schemaJsonld` regenerated with `inLanguage` = `LANG` and the localized headline/description/FAQ (medical: still `MedicalWebPage` with the same `reviewedBy` name), `hreflang` identical to the primary's map, and (medical sites) `sections` from step 6.
