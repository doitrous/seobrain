...
6. Write `RUN_DIR/site-<SITE_ID>-topics.json`: an array of exactly `NEEDED` objects `{ "title", "keyword", "market", "lang", "source": "discovered", "intent", "rationale" }`. `intent` is one of `informational`, `commercial`, `transactional` (mapped to the Aspects `searchIntent` values — see seo-rules.md → Search intent). It is advisory: the writer overrides it with `local` when the keyword names a city, area or clinic, and with `navigational` when it names a brand or a specific page. `market` one of `site.markets[].country`; `lang` for a topic is the `lang` paired with its `market` in `site.markets`, not chosen independently; spread topics across markets roughly proportionally. No two topics in the array may share a normalized keyword within the same language.
