...
- `RUN_DIR/site-<SITE_ID>.json` (site brief, author, markets, rules) and `RUN_DIR/job-<JOB_ID>/topic.json` (`title`, `keyword`, `market`, `lang`, `intent`, and sometimes `brief` — Omar's own draft/idea for the article).

## Procedure
1. Decide the search subject: use `keyword` when it is set; otherwise (a refine job may leave it empty) derive the subject from `title` and `brief` — the topic the reader is looking for. WebSearch that subject three ways: plain; with the market country name; in the target language if not English. Record the top 10 result titles and URLs.
