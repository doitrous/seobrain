...
- **Forced ideas first.** For every entry in `forcedTopics` (Omar ticked "force this idea next run"), create a job **unconditionally — even when `neededThisWeek` is 0** — treating the row exactly like a `queuedTopics` one (set `topicId = forcedTopics[].id`; carry its `brief`, `keyword`, `title`, `lang`, `market`; same `intent` rule; a non-empty `brief` still means MODE=refine). Let `forcedCreated` be how many you created (under `--resume`, skip any whose `topicId` is already in `state.jobs` and still count them). Forced ideas count toward the week first and are guaranteed even over cadence.
- Take up to `max(0, neededThisWeek − forcedCreated)` entries from `queuedTopics` (they are user-supplied). Each `queuedTopics[]` row has `id, title, keyword, market, lang, source, brief`: set `topicId = queuedTopics[].id`, and take `lang`, `market`, `keyword`, `title` from that same row (matched by topicId). Set `intent` to `informational`, unless the keyword clearly signals cost or comparison — then use `commercial`. **Take rows with `source: "refresh"` first** — the hub queued them because a published article reached its refresh date; they carry `refreshJobId`, the job being refreshed, and they are exempt from the keyword guard. Write these to topic.json the same way as a discovered topic.
  - **`brief` is Omar's own idea or rough draft for the article.** When `queuedTopics[].brief` is a non-empty string, this is a refine job: the writer builds the article on top of that draft instead of researching a topic from scratch (§3, MODE=refine). Such a row may leave `keyword` and/or `title` empty on purpose — pass them through as-is (empty string), and the researcher and writer derive them from the `brief`. Always copy `brief` verbatim into topic.json.
- If still short by `k` (of `max(0, neededThisWeek − forcedCreated)` — forced jobs already fill the week), dispatch **topic-scout** with RUN_DIR, SITE_ID, `NEEDED=k`; read `site-<id>-topics.json`.
...
- Create jobs: for a queued topic write `{ "siteId", "topicId" }` — and when that topic's `source` is `"refresh"`, write `{ "siteId", "topicId", "refreshOf": <topic.refreshJobId> }` instead. For a discovered one write `{ "siteId", "topic": {…} }`. Write it to `RUN_DIR/job-tmp.json` and run `scripts/hub.sh create-job RUN_DIR/job-tmp.json`; it prints the job JSON — JOB_ID is its `id`.
  ...
  - Otherwise run `mkdir -p RUN_DIR/job-<JOB_ID>`, write `topic.json` there (title, keyword, market, lang, intent, source, **brief** — use `""` when the topic has none), and add to `state.jobs[JOB_ID] = { "siteId", "topicId" (queued topics only), "title", "lang", "languages": site.languages, "refreshOf": <job id or null>, "steps": {}, "auditLoops": 0, "status": "planned" }`.
...
## 3. Write
For every job with research done and no `steps.draft`: dispatch **writer** with `MODE=write` — or with `MODE=refresh` when the job has `refreshOf` set (its topic came from the refresh queue with `source: "refresh"`, and `refreshOf` is that topic's `refreshJobId`, the original job) — or with `MODE=refine` when the job's `topic.json` has a non-empty `brief` (Omar wrote an idea/draft for it; the writer builds the article on top of it). A job never has both `refreshOf` and a `brief`; if it somehow does, `refresh` wins. The prompt contract is unchanged: `RUN_DIR`, `SITE_ID`, `JOB_ID`, `MODE`; in refresh mode the writer fetches the published article itself with `scripts/hub.sh articles` and revises it; in refine mode the writer reads `topic.json.brief` as the starting point. On ok set `steps.outline`, `steps.draft`, `steps.image_brief` to `"done"`. On fail apply the failure policy.
