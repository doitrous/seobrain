...
- `topic.json` (topic-scout → orchestrator): `{ "title", "keyword", "market", "lang", "source": "discovered", "intent": "informational|commercial|transactional", "rationale" }`. For a queued topic the orchestrator also writes `"brief"` (Omar's own idea/draft, `""` when none) and, for a user idea, `"source": "user"`; a non-empty `brief` makes it a refine job (writer `MODE=refine`) and may leave `keyword`/`title` empty for the brain to derive.
