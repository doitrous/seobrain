...
- Meta title ≤ 60 characters (aim 50–58 — the hub warns `title_length_aim` above 58), Arabic ≤ 70. Meta description 130–155 characters, Arabic 110–180. Meta title ≠ H1 wording exactly; it may add a hook ("2026 guide", "costs & clinics").
...
| a single product or service item ("long navy blue bridesmaid dress", "3-day Cairo trip") | `product` (v2 phase 8) |
...
- `pillar`: an overview that introduces and links every member of its hub (member titles/slugs come from the brief); no facts of its own beyond what is needed to summarize each member. `schemaJsonld` swaps `BlogPosting` for `CollectionPage` + `ItemList` of the members.
...
- `product` (v2 phase 8): H1 = the product/service name as the keyword ("Long Navy Blue Bridesmaid Dress"); ≥ 300 words covering what it is, who it's for, what's included/not, price with currency and an "as of" date, delivery/duration, care/recovery, FAQ. Never copy manufacturer/supplier text. `schemaJsonld` swaps `BlogPosting` for `Product` + `Offer`. Links: the pillar (category), 2 related products (siblings), 1 money page (booking/contact).
...
- `cta`: the closing section must contain the CTA text verbatim — the job's market `cta.text` when its `site.markets` entry sets one (see Market rules), otherwise `site.cta.text` — or a markdown link to that CTA's `url`. Do not invent a CTA — use the site's or the market's own text/url, never a paraphrase.
...
- **`cta`** (optional `{ text, url }`): overrides `site.cta` for this job when set — use the market's CTA text/URL instead of the site's (see the `cta` rule above).
...
Writer, localizer and auditor all read the job's market entry before writing or checking a word. A market with no `rules`/`glossary`/`cta` simply falls back to the site defaults for those fields — nothing to override.
